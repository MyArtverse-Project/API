...
const sendUnauthorizedResponse = (reply: FastifyReply, message: string = 'Unauthorized') => {
...
const refreshTokenLogic = async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  try {
    const payload = await request.server.jwt.verify<{ id: string }>(refreshToken) as UserPayload;
